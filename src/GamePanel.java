...
public class GamePanel extends JPanel implements ActionListener{
	...
	public void paintComponent(Graphics g) {
		...
		if(game_running) {
			timer.setDelay(DELAY); // Resetto il
			//moving player
			...
			for(int i = 0 ; i < enemies.size(); i++){
				...
				enemies.get(i).pathFinding(player, map);
				enemies.get(i).move();
			...
			timer.setDelay(DELAY*8); // Resetto il
			mainMenu(g);
		}
	}

	public void mainMenu(Graphics g){
        delayBetweenImages ++; // Tempo di visualizzazione predefinito per ogni immagine (2 secondi)
		if (delayBetweenImages >= 50) delayBetweenImages = 1;
		g.drawImage(menu_gif[delayBetweenImages],0,0, menu_gif[delayBetweenImages].getWidth(null), menu_gif[delayBetweenImages].getHeight(null), this);
		try {
	...
	public class MyKeyAdapter extends KeyAdapter{
		...
		public void keyPressed(KeyEvent e) {
			...
			if (e.getKeyCode() == KeyEvent.VK_W)
				P_up = (int)player.MAXSPEED/10;

			if (e.getKeyCode() == KeyEvent.VK_A)
				P_left = (int)player.MAXSPEED/10;

			if (e.getKeyCode() == KeyEvent.VK_S)
				P_down = (int)player.MAXSPEED/10;

			if (e.getKeyCode() == KeyEvent.VK_D)
				P_right = (int)player.MAXSPEED/10;
